    ...
    <div className="flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-100 to-purple-400">
      ...
      <div className="bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm">
        ...
        <form >
          ...
          <button className="text-white font-bold  w-full py-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-900">{state }</button>
        </form>
        {state === "Sign Up" ? (<p className="text-gray-400 text-center text-xs mt-4">Alreday have an account?{' '}<span onClick={()=>setstate("Login")} className="text-blue-400 cursor-pointer underline">Login here</span></p>) :
